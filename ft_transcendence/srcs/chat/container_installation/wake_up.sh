...
#    Updated: 2025/06/27 12:11:54 by rbulanad         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

# path : srcs/chat/container_installation/wake_up.sh
...
while [ ! -f "$PATH_DB_VOLUME/database.db" ]; do
	echo -e ${LLYELLOW}${BOLD}'CHAT Waiting for database file to be available.'${CLEAN}
    sleep 1
done
echo -e ${LGREEN}${BOLD}'CHAT Database file is available !'${CLEAN}

echo -e ${LBLUE}${BOLD}'launching project at ['"$PROJECT_CODE_DIRECTORY"'/server.js] :'${CLEAN}
# echo -e -n ${LLYELLOW}
node --experimental-modules $PROJECT_CODE_DIRECTORY/server.js
echo -e ${LGREEN}${BOLD}' '${CLEAN}

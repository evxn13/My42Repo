...
import backgroundImage from '../../website/src/assets/Background-create.jpg';
...
const GamePageCreate: React.FC = () =>
{
	let		roomUniqueId	= uuidv4();
	const	location		= useLocation();

	const	[renderMap, sendMessageToGameBackendWs, clearRenderMap, setRenderMap] = useContext(BackendWsContext);
	...
	const	[invitedPlayer, setInvitedPlayer]		= useState<string | null>(null);

	// Récupérer le joueur invité depuis l'URL
	useEffect(() => {
		const params = new URLSearchParams(location.search);
		const playerToInvite = params.get('invitedPlayer');
		if (playerToInvite) {
			setInvitedPlayer(playerToInvite);
			// Modifier le nom de la room pour inclure l'invitation
			const inviteRoomName = `Game with ${playerToInvite}`;
			setRoomNameInput(inviteRoomName);
			roomName = inviteRoomName;
			roomCreationEvent.setRoomName(inviteRoomName);
		}
	}, [location.search]);
	...
	useEffect(() => {
		if (selectedShape) {
			switch (selectedShape) {
				case 'triangle':
					roomCreationEvent = RoomCreationEventWS.createRoomTriangle(roomName, numberOfPlayers);
					break;
				case 'square':
					roomCreationEvent = RoomCreationEventWS.createRoomSquare(roomName, numberOfPlayers);
					break;
				case 'parallelogram':
					roomCreationEvent = RoomCreationEventWS.createRoomParallelogram(roomName, numberOfPlayers);
					break;
				default:
					roomCreationEvent = RoomCreationEventWS.createRoomRectangle(roomName, numberOfPlayers);
					break;
			}
		} else {
	...
	let sendMapToBackend = function(roomCreationEvent_ = roomCreationEvent): void
	{
		...
		// Si un joueur est invité, envoyer l'invitation
		if (invitedPlayer)
			sendGameInvitation(roomCreationEvent_.roomName, invitedPlayer);
	...
	// Fonction pour envoyer l'invitation via API REST au lieu de WebSocket
	const sendGameInvitation = async (roomId: string, playerUsername: string) => {
		try {
			console.log('🎮 Sendin game invitation...', { roomId, playerUsername });
			
			// Récupérer le token d'authentification
			const token = localStorage.getItem('token');
			if (!token) {
				console.error('❌ Authentification token missing');
				return;
			}

			// Récupérer le nom d'utilisateur de l'expéditeur depuis le token
			...
			const response = await fetch(`https://${import.meta.env.VITE_HOST_TRANSCENDENCE}:${import.meta.env.VITE_PORT_NGINX}/req/game-invite`, {
				method: 'POST',
				headers: {
				...
				body: JSON.stringify({
					...
					to: playerUsername,
					roomId: roomId,
					message: `${fromUsername} invite you to join game: ${roomId}`
				})
			});

			if (response.ok) {
				console.log('✅ Game invite send with success with API REST');
			} else {
				console.error('❌ Error while sending invite:', response.status, response.statusText);
			}
		} catch (error) {
				...
				<div className="min-h-screen flex items-center justify-center">
					<div className="flex flex-row mobile:flex-col items-center gap-8 p-4">
						...
						<div className="bg-black/20 flex flex-col items-center sm:justify-around backdrop-blur-sm border-1 border-white/30 rounded-2xl mt-20 sm:mt-0 sm:h-[600px] shadow-lg p-8 w-full sm:w-fit">
							...
							<div className="mt-8">
								<h3 className="text-white text-lg mb-4 text-center font-semibold">Number of players(ShapeOnly)</h3>
								...
								<button 
										...
										className={`relative w-20 h-20 rounded-full bg-white/10 text-white hover:bg-white/20 transition-all duration-300 
											${numberOfPlayers === 2 ? 'ring-4 ring-white/70 bg-white/20' : ''}`}
									>
									...
									<button 
										...
										className={`relative w-20 h-20 rounded-full bg-white/10 text-white hover:bg-white/20 transition-all duration-300 
											${numberOfPlayers === 3 ? 'ring-4 ring-white/70 bg-white/20' : ''}`}
									>
										<div className="absolute inset-0 flex items-center justify-center">
									...
									<button 
										...
										className={`relative w-20 h-20 rounded-full bg-white/10 text-white hover:bg-white/20 transition-all duration-300 
											${numberOfPlayers === 4 ? 'ring-4 ring-white/70 bg-white/20' : ''}`}
									>
										<div className="absolute inset-0 flex items-center justify-center">
						...
						{/* Indicateur du joueur invité */}
						{invitedPlayer && (
							<div className="mb-6 p-4 bg-green-500/20 border border-green-500/50 rounded-lg">
								<h3 className="text-green-400 text-lg font-semibold mb-2">🎮 Sending invitation</h3>
								<p className="text-white text-sm">
									You invite <span className="font-bold text-green-300">{invitedPlayer}</span> to join this game.
								</p>
								<p className="text-green-200 text-xs mt-1">
									Invitation while be send after room creation.
								</p>
							</div>
						)}
